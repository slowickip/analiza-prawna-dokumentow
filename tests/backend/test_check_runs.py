...
@pytest.mark.parametrize("change", [None, "configuration", "totals"])
def test_documented_offline_parity_replay(
    ...
    for name in (
        ...
        shutil.copy2(root / "evaluation-data" / name, target)
    shutil.copytree(root / "evaluation-data/results/final", data / "results/final")
    if change == "configuration":
        import hashlib

        protocol_path = data / "scoring-protocol.json"
        protocol = json.loads(protocol_path.read_text())
        series = protocol["scored_runs"]["series"][0]
        relative = "ED-005/mid.json"
        path = tmp_path / series["batch"] / relative
        run = json.loads(path.read_text())
        run["concurrency"] += 1
        path.write_text(json.dumps(run), encoding="utf-8")
        series["sha256"][relative] = hashlib.sha256(path.read_bytes()).hexdigest()
        protocol_path.write_text(json.dumps(protocol), encoding="utf-8")
    elif change == "totals":
        path = data / "results/parity.json"
        report = json.loads(path.read_text())
        report["summary"]["within_series"]["checked"] = 0
        path.write_text(json.dumps(report), encoding="utf-8")

    section = (
        (root / "evaluation-data/README.md")
        .read_text()
        .split("### Reproduce the analyzer parity check", 1)[1]
    )
